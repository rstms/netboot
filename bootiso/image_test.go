...
func TestCreateOpenBSDISO(t *testing.T) {
	...
	sourceImage, err := filepath.Abs(filepath.Join("testdata", "openbsd.iso"))
	require.Nil(t, err)
	if IsFile(sourceImage) {
		err = os.Remove(sourceImage)
		require.Nil(t, err)
	}
	err = files.UnzipFileFromFS(sourceImage, path.Join("ipxe", "openbsd-7.7-amd64.iso.gz"), template.Ipxe)
	require.Nil(t, err)
	...
	fooFile := filepath.Join("testdata", "foo")
	writeTestFile(t, fooFile)

	rootFiles := []string{autoexecFile, fooFile}
	err = CreateNetbootISO(outputImage, sourceImage, efiImage, rootFiles)
	require.Nil(t, err)
}
